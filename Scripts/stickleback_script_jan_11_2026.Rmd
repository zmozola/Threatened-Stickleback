...
## Additional Formula Setup Again
```{r Additional Formula Setup}
...
#NOT working something wrong with my sex, has more than 2 levels
for (j in names(sexyunarmouredlakes)){
  lake <- sexyunarmouredlakes[[j]]
  for (i in numeric_traits_unarmoured) {
    for (k in timeperiods) {
      #lake <- lake$time_period == k
      #test <- t.test(lake[[i]] ~ sex, data = lake)
      #if (test$p.value < 0.05) {
        print(paste(j,i,k, nlevels(sex)))
        #print(test)
    }}}#}

#Sex NO time period only sig ALSO MESSED UP, somethign to do with ????
for (j in names(sexyunarmouredlakes)){
  lake <- sexyunarmouredlakes[[j]]
  for (i in numeric_traits_unarmoured) {
    test <- t.test(lake[[i]] ~ time_period, data = lake)
...
for (j in names(sexygiantlakes)){
  ...
  for (i in spines) {
    for (k in timeperiods) {
      #lake <- lake %>% filter(time_period == k) HAVING TROUBLE W SUBSET
      test <- t.test(lake[[i]] ~ sex, data = lake)
      if (test$p.value < 0.05) {
        print(paste(j,i,k, nlevels(sex)))
        print(test)

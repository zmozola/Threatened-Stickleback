...
for (j in names(sexylakes)) {
  sexylake <- sexylakes[[j]]
  sexylakecolour <- lakecolours[[j]]
  for (i in numeric_traits) {
    print(ggplot(sexylake, aes(x =  time_period, y = .data[[i]], color = sex)) +
            geom_boxplot(fill = sexylakecolour) + 
            theme_minimal() +  
            labs(title = j))
}}
...
for (j in names(sexylakes)) {
  ...
  sexylakecolour <- lakecolours[[j]]
  for (i in numeric_traits) {
    print(ggplot(sexylake, aes(x =  as.factor(year), y = .data[[i]], color = sex)) +
            geom_boxplot(fill = sexylakecolour) + 
            theme_minimal() +  
            labs(title = j))
}}

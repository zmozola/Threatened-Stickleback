...
lakes2 <- c("drizzle", "mayer", "rouge", "boulton", "serendipity")
...
for (i in numeric_traits_2) {
    print(ggplot(rouge, aes(x = sl, y = .data[[i]])) + geom_point(color = "purple") + geom_text(aes(label = fish_id), color = "purple"))
}

#Figure out how to loop lakes
for (j in lakes2) {
    print(ggplot(j, aes(x = sl, y = body_depth.adj)) + geom_point() + geom_text(aes(label = fish_id)))
}
